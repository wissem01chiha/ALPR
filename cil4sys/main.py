...
from cil4sys import threshold 
from cil4sys import video 
from cil4sys import countour 
from cil4sys import metric
from cil4sys import track
from cil4sys.track import process_video, process_frame
...
def main(video_path=None):
  ...
  if video_path:
     # VIDEO_PATH = os.path.join(path["video_path"], 
      #          video_path) if video_path else path["video_path"]
      ...
      try:
        ...
        process_video(video_path, ocr_type, tracker, recorded_license_plates)
        # End timer
        end_time = time.time()
      ...
      CAPTURE_SUCCESS=video.capture_video(10,5,(480,640),VIDEO_FRAMES_PATH, VIDEO_PATH)
      if CAPTURE_SUCCESS:
          ...
          print("[ERROR] Video capture failed. Check the camera. Right now default video will be proceeded.")
      ...
      for frame_index, frame_id in enumerate(os.listdir(VIDEO_FRAMES_PATH)) :
          ...
          if background_frame is not None:
              # detect motion in video/image? frames 
      ...
      for  roi_frame_id in sorted_dict.keys() :
          ...
          contours, _ = cv2.findContours(binary_frame, 
          ...
          for i, matched_chars in enumerate(matched_result):
              ...
              img_rotated = cv2.warpAffine(binary_frame, M=rotation_matrix, 
                            dsize=(width, height))
